...
- Change `SECRET_KEY` and credentials in `.env`
- Consider:
  - storing hashed passwords (not plain credentials)
  - a stronger DB (PostgreSQL) + migrations
  - serving `frontend/dist` via a web server (nginx) and reverse-proxying requests to FastAPI
